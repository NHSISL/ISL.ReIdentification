...
namespace ISL.ReIdentification.Core.Tests.Unit.Services.Foundations.Documents
{
    public partial class DocumentsTests
    {
        ...
        [InlineData("")]
        [InlineData(" ")]
        public async Task ShouldThrowValidationExceptionOnAddFolderIfArgumentsInvalidAndLogItAsync(
            string invalidString)
        {
            ...
            ValueTask addFolderTask =
                this.documentService.AddFolderAsync(invalidFileName, invalidContainer);

            DocumentValidationException actualDocumentValidationException =
                await Assert.ThrowsAsync<DocumentValidationException>(addFolderTask.AsTask);
