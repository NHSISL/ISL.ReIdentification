...
namespace ISL.ReIdentification.Core.Tests.Unit.Services.Foundations.OdsDatas
{
    ...
    {
        [Fact]
        public async Task ShouldThrowCriticalDependencyExceptionOnRetrieveChildrenByParentIdIfSqlErrorOccursAndLogItAsync()
        {
            // given
            ...
            ValueTask<List<OdsData>> retrieveOdsDataByIdTask =
                this.odsDataService.RetrieveChildrenByParentId(someId);
        ...
        [Fact]
        public async Task ShouldThrowServiceExceptionOnRetrieveChildrenByParentIdIfServiceErrorOccursAndLogItAsync()
        {
            // given
            Guid someId = Guid.NewGuid();
