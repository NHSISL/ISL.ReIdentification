    ...
    public partial class UserAgreementsControllerTests
    {
        ...
        public async Task ShouldReturnRecordOnGetByIdsAsync()
        {
            ...
            userAgreementServiceMock
                .Setup(service => service.RetrieveUserAgreementByIdAsync(It.IsAny<Guid>()))
                    .ReturnsAsync(storageUserAgreement);

            // when
            ActionResult<UserAgreement> actualActionResult =
                await userAgreementsController.GetUserAgreementByIdAsync(inputId);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            userAgreementServiceMock
                .Verify(service => service.RetrieveUserAgreementByIdAsync(It.IsAny<Guid>()),
                    Times.Once);
