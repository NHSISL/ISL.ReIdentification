    ...
    public partial class OdsDataControllerTests
    {
        ...
        public async Task ShouldReturnBadRequestOnPostIfValidationErrorOccurredAsync(Xeption validationException)
        {
            ...
            this.odsDataServiceMock.Setup(service =>
                service.AddOdsDataAsync(It.IsAny<OdsData>()))
                    .ThrowsAsync(validationException);

            // when
            ActionResult<OdsData> actualActionResult =
                await this.odsDataController.PostOdsDataAsync(someOdsData);
        ...
        [Fact]
        public async Task ShouldReturnConflictOnPostIfAlreadyExistsOdsDataErrorOccurredAsync()
        {
            // given
            OdsData someOdsData = CreateRandomOdsData();
